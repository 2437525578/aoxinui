...
const handleUnauthorized = () => { /* ... */
    ...
    sessionStorage.removeItem('authToken'); sessionStorage.removeItem('username'); sessionStorage.removeItem('userRole');
    router.push('/login');
};
...
const handleUpdate = (row) => { /* ... (之前的 handleUpdate 代码) ... */ // row 参数现在不再从行内按钮传递，而是始终为 undefined
    resetForm();
    const targetRow = inventoryItems.value.find(item => item.id === selectedIds.value[0]); // 只处理顶部按钮逻辑
    if (!targetRow) { ElMessage.warning('请选择一条数据进行修改'); return; }
    isEditMode.value = true; dialogTitle.value = '编辑库存记录'; itemForm.id = targetRow.id; itemForm.chemical = targetRow.chemical; itemForm.location = targetRow.location; itemForm.quantity = parseFloat(targetRow.quantity) || 0.00; itemForm.unit = targetRow.unit; itemForm.batch_number = targetRow.batch_number; itemForm.purchase_date = targetRow.purchase_date; itemForm.expiry_date = targetRow.expiry_date;
     if(targetRow.chemical_details) { chemicalOptions.value = [{ id: targetRow.chemical, nameCn: targetRow.chemical_details.nameCn, casNo: targetRow.chemical_details.casNo }]; } else { chemicalOptions.value = []; }
