...
const login = async (username, password, captcha) => {
  const res = await axios.post('https://127.0.0.1:5000/api/user/login', {
    username: username,
    password: password,
    captcha: captcha, // 添加验证码参数
  })
  if (res.data.status === 'success') {
    router.push('/dashboard')
    // localStorage.setItem('username', username) // 移除直接存储用户名
    // 假设后端返回一个token，并将其存储起来
    if (res.data.token) {
      localStorage.setItem('authToken', res.data.token); // 存储认证令牌
    }
    // 记住密码逻辑（待优化）
    if (loginForm.rememberPassword) {
      ...
      localStorage.removeItem('authToken'); // 如果不记住密码，也移除令牌
    }
  } else {
